...
import com.stepstracker.android.data.run.RunRepository
...
class StepsTrackerApp:Application() {
    lateinit var database:StepsDatabase;lateinit var session:SessionStore;lateinit var cache:LocalCache;lateinit var server:ServerSettings;lateinit var trackingSettings:TrackingSettings;lateinit var api:ApiClient;lateinit var steps:StepsRepository;lateinit var runs:RunRepository
    override fun onCreate() {
        super.onCreate();org.maplibre.android.MapLibre.getInstance(this);database=StepsDatabase.create(this);session=SessionStore(this);cache=LocalCache(this);server=ServerSettings(this);trackingSettings=TrackingSettings(this);api=ApiClient(session,server)
        ...
        runs=RunRepository(database.runs(),api,deviceId)
        ...
        WorkManager.getInstance(this).enqueueUniquePeriodicWork("runs-sync",ExistingPeriodicWorkPolicy.UPDATE,PeriodicWorkRequestBuilder<RunSyncWorker>(15,TimeUnit.MINUTES).setConstraints(Constraints.Builder().setRequiredNetworkType(NetworkType.CONNECTED).build()).build())
    }
}

class RunSyncWorker(context:android.content.Context,params:WorkerParameters):CoroutineWorker(context,params) { override suspend fun doWork():Result { val app=applicationContext as StepsTrackerApp;return if(runCatching { app.runs.syncAll() }.isSuccess)Result.success() else Result.retry() } }
