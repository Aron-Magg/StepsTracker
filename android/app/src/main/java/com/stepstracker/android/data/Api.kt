...
class ApiClient(private val session: SessionStore) {
    ...
    private suspend inline fun <reified T> request(path:String, block:HttpRequestBuilder.()->Unit={}):T {
        var response=client.request(BuildConfig.API_BASE_URL+path) { session.accessToken?.let { bearerAuth(it) }; block() }
        if(response.status==HttpStatusCode.Unauthorized && session.refreshToken!=null && refresh()) response=client.request(BuildConfig.API_BASE_URL+path) { bearerAuth(session.accessToken!!); block() }
    ...
    private suspend fun refresh():Boolean = runCatching {
        val result=client.post(BuildConfig.API_BASE_URL+"api/v1/auth/refresh") { contentType(ContentType.Application.Json);setBody(RefreshRequest(session.refreshToken!!)) }
        if(!result.status.isSuccess()) false else { session.save(result.body());true }
    }.getOrDefault(false)
    suspend fun logout() { session.refreshToken?.let { runCatching { client.post(BuildConfig.API_BASE_URL+"api/v1/auth/logout") { contentType(ContentType.Application.Json);setBody(LogoutRequest(it)) } } };session.clear() }
